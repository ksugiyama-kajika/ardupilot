...
local stage = 0
local circle_angle = 180
local circle_angle_increment = 0.1    -- increment the target angle by 1 deg every 0.1 sec (i.e. 10deg/sec)
  ...
  if not arming:is_armed() then -- reset state when disarmed
    ...
    if pwm6 and pwm6 > 1800 then    -- check if RC6 input has moved high
      ...
      elseif (stage == 3 ) then   -- Stage3: fly a vertical circle
        ...
        if (circle_angle >= 540) then
          stage = stage + 1
        end
        local target_vel = Vector3f()
          -- local vel_xy = math.cos(math.rad(circle_angle)) * circle_speed
        -- local vel_xy = 2 * math.sin(math.rad(circle_angle)) * (2 * (math.cos(math.rad(circle_angle)))^3 - 1)
        local vel_xy = (-48 * (math.sin(math.rad(circle_angle))^2) * math.cos(math.rad(circle_angle))) 
        target_vel:x(yaw_sin * vel_xy)
        target_vel:y(yaw_cos * -vel_xy)
          -- target_vel:z(-math.sin(math.rad(circle_angle)) * circle_speed)
        -- target_vel:z(48 * (math.sin(math.rad(circle_angle))^2) * math.cos(math.rad(circle_angle)))
        target_vel:z(-2 * math.sin(math.rad(circle_angle)) * (2 * (math.cos(math.rad(circle_angle)))^3 - 1))
